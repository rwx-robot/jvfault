...
/**
 * MQTT 传输单元测试（不依赖真实 broker；broker 集成测试脚本在源码注释说明）。
 *
 * @since v0.6.0 (2020)
 */
@DisplayName("MQTT 传输单元测试")
class MQTTTransportTest {
    ...
    void testLifecycle() {
        ConnectionConfig config = new ConnectionConfig("localhost:5672")
                .option("user", "admin")
                .option("virtualHost", "/");
        MQTTTransportServer server = new MQTTTransportServer(config);
        MQTTTransportClient client = new MQTTTransportClient(config);
    ...
    void testWireFields() throws Exception {
        ...
        com.jvfault.microservices.Message original = new com.jvfault.microservices.Message(
                "mqtt.hello", codec.encode("ping"), "corr-1", null);
        assertEquals("mqtt.hello", original.getPattern());
