...
/**
 * RabbitMQ 传输集成测试（真实 broker）。
 *
 * <p>bootstrap 来源优先级：环境变量 {@code JVFAULT_RMQ_BOOTSTRAP}（CI service）
 * → 本地 docker 容器（rabbitmq:3-management）→ 不可用则跳过。
 *
 * @since v1.0.2 (2026)
 * @author jvfault team
 */
@DisplayName("RabbitMQ 传输集成测试（真实 broker）")
class RabbitMQIntegrationTest {
    ...
    static DockerContainer rmq;
    static String bootstrap;
    static boolean brokerUp;

    @BeforeAll
    static void startRmq() throws Exception {
        String external = System.getenv("JVFAULT_RMQ_BOOTSTRAP");
        if (external != null && !external.trim().isEmpty()) {
            ...
            if (!isReachable(bootstrap)) {
                brokerUp = false;
                System.err.println("[rmq-it] broker 不可达，跳过集成测试: " + bootstrap);
                return;
            }
        } else if (DockerContainer.dockerAvailable() && DockerContainer.hasImage()) {
            rmq = new DockerContainer().start();
            bootstrap = rmq.getBootstrap();
        } else {
            brokerUp = false;
            return;
        }
    ...
    @Test
    @DisplayName("端到端 request-reply（topic exchange + replyTo 队列）")
    void testRequestReply() throws Exception {
        assumeTrue(brokerUp, "需要 RabbitMQ broker（Docker 或 JVFAULT_RMQ_BOOTSTRAP）");

        JacksonMessageCodec codec = new JacksonMessageCodec();
        RabbitMQTransportServer server = new RabbitMQTransportServer(new ConnectionConfig(bootstrap));
        ...
        TransportClient client = new RabbitMQTransportClient(new ConnectionConfig(bootstrap));
    ...
    void testPublishEvent() throws Exception {
        ...
        RabbitMQTransportClient client = new RabbitMQTransportClient(new ConnectionConfig(bootstrap));
